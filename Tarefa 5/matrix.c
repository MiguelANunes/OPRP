...
// Cria matriz de dimensão rows X cols.
	int i;
	double *data;
	...
	m->rows = rows;
	m->cols = cols;
	m->data = (double **) malloc(sizeof(double *) * rows);  // m->data é um ponteiro p/ rows ponteiros de doubles
	data = (double *) malloc(sizeof(double) * rows * cols); // data é um ponteiro p/ rows * cols doubles
	for (i = 0; i < rows; i++) {
		m->data[i] = &data[cols * i]; // m->data[i] aponta para data[cols*i]
	}
	return m;
}
...
// Preenche matriz com uma sequencia de números aleatórios.
void matrix_randfill(matrix_t *m){
	int i, j;
	for (i = 0; i < m->rows; i++) {
		for (j = 0; j < m->cols; j++) {
			m->data[i][j] = random();
			// vai de 1 até RAND_MAX
		}
	}
}
...
matrix_t *matrix_multiply(matrix_t *A, matrix_t *B){
	...
	assert(A->cols == B->rows); // garantindo que é possível multiplicar
				   // rows     cols
	m = matrix_create(A->rows, B->cols); // cria a matriz resultado
	count = A->cols;
	...
	for (i = 0; i < m->rows; i++) {
		for (j = 0; j < m->cols; j++) {
			sum = 0.0;
			for (k = 0; k < count; k++) {
				// nova célula é linha de uma vezes coluna de outra
...
{
	matrix_t *m = matrix_create(block->row_end - block->row_start, 
								block->col_end - block->col_start);
	int i, j;
	for (i = 0; i < m->rows; i++) {
		for (j = 0; j < m->cols; j++) {
...
void matrix_block_copy(matrix_t *source, matrix_t *dest, int source_row, 
					   int source_col, int dest_row, int dest_col, int rows,
					   int cols){
	// dest_row, dest_col, source_row e source_col são offsets
	int i, j;
	for (i = 0; i < rows; i++) {
		for (j = 0; j < cols; j++) {
			dest->data[i + dest_row][j + dest_col] =
				source->data[i + source_row][i + source_col];
		}
	}
}
...
// Aloca espaço para uma partição da matriz (n blocos)
...
void partition_print(block_t *part){
	printf("[%d:%d][%d:%d]\n", 
			 part->row_start, part->row_end, 
			 part->col_start, part->col_end);
}
...
block_t *matrix_row_partition(matrix_t *m, int n){
	...
	int nrows;
	int size = m->rows / n;
	int extra = m->rows % n; // quantas linhas incompletas sobram
	int base = 0;
	block_t *part = partition_create(n);
	// cria um array de partições
